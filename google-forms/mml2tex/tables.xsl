...
<xsl:template mode="mml2tex" match="m:mtd[@columnspan]">
...
<xsl:template mode="mml2tex" match="m:mtd">
...
<xsl:template mode="mml2tex" match="m:mtr">
	<xsl:apply-templates mode="mml2tex"/>
	<xsl:if test="count(following-sibling::*)>0">
		<xsl:text>\\ </xsl:text>
	</xsl:if>
...
<xsl:template mode="mml2tex" match="m:mtable">
